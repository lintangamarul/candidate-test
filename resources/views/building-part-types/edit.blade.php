...
@section('title', 'Edit Building Part Type - CLT Toolbox')
@section('page-title', 'Edit Building Part Type')
...
<div class="bg-white rounded-lg shadow">
    <div class="p-6">
        <form action="{{ route('building-part-types.update', $buildingPartType) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           name="name" 
                           id="name" 
                           value="{{ old('name', $buildingPartType->name) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('name') border-red-500 @enderror"
                           placeholder="Enter building part type name"
                           required>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea name="description" 
                              ...
                              placeholder="Enter building part type description"
                              required>{{ old('description', $buildingPartType->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" 
                           name="quantity" 
                           id="quantity" 
                           value="{{ old('quantity', $buildingPartType->quantity) }}"
                           min="0"
